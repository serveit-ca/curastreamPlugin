...
span.deletePhaseEdit.glyphicon.glyphicon-trash, span.deleteExerciseEdit.glyphicon.glyphicon-trash, span.deletePhase, span.deleteExercise {
    color: #555;
    font-size: 10px;
    display: inline-block;
    margin-left: 10px;
...
<body>
	<?php 
		...
		if (isset($_POST['addProgram']) && $_POST['addProgram'] == 'Add Program') {
			...
			$dataProg = array(
				...
				'equipment' => ucfirst($_POST['progEquip']),
			...
			foreach ($_POST['phase'] as $key => $value) {
				...
				$phaseIntro = $value['intro'];
				$phaseDuration = $value['duration'];
				$phaseNotes = $value['notes'];
				...
				foreach ($value['exercise'] as $key_s => $value_s) {
					...
					$exerciseVar = $value_s['variation'];
					$exerciseEquip = $value_s['equipment'];
					$exerciseIns = $value_s['instructions'];
		...
		if (isset($_POST['updateProgram']) && $_POST['updateProgram'] == 'Update Program') {
			...
			$dataProgUpdate = array(
				...
				'name' => ucfirst($_POST['progNameupdate']),
				'description' => ucfirst($_POST['progDescUpdate']),
				'duration' => $_POST['progDurationUpdate'],
				'equipment' => ucfirst($_POST['progEquipUpdate']),
				'weekly_plan' => ucfirst($_POST['progPlanUpdate']),
				'life_style' => ucfirst($_POST['progLifestyleUpdate']),
				'assoc_body_part_id'=> $idsBody,
				'how_it_happen' => $idsHow,
				'sports_occupation' => $idsSo,
			...
			foreach ($_POST['phase'] as $key_p => $value_p) {
				...
				elseif ($value_p['phaseId'] == '') { 
					$phaseName = $value_p['nameUpdate'];
					$phaseIntro = $value_p['introUpdate'];
					$phaseDuration = $value_p['durationUpdate'];
					$phaseNotes = $value_p['notesUpdate'];
					$tablePhase = 'dev_cura_phases';
					$dataPhase = array(
	...
	if (isset($_POST['editprogram']) && isset($_POST['edit']) && $_POST['edit'] == 'Edit' ) {
		...
		foreach ($getPhaseByProgId as $key => $value) {
			$phaseID = $value['id'];
			$getPhaseByProgId[$key]['exercise'] = $getExerByPhaseId = $wpdb->get_results("SELECT * FROM `dev_cura_exercises` WHERE phase_id = $phaseID", ARRAY_A);
		}
		// echo "<pre>";
		...
		foreach ($getPhaseByProgId as $key => $value) {
			...
			$getExerByPhaseId = $wpdb->get_results("SELECT * FROM `dev_cura_exercises` WHERE phase_id = $phaseID", ARRAY_A);
			// echo "<pre>";
			...
			<div class="row">
				...
				<div class="col-md-6">
					...
					<div class="form-group">
						<div>
						    ...
						    <input type="text" name="thumbUpdate" id="image_url" class="regular-text form-control" value="<?php echo $getProgramById[0]['thumbnail'] ?>">
				...
				<div class="col-md-6">
					<div class="form-group">
						<select multiple="multiple" name="associated_body_parts_update[]" class="form-control multiple_parts" required="required">							
							...
							<option value="<?php echo array_values((array)$value)[0]; ?>" <?php echo in_array(array_values((array)$value)[0], $assoc_parts) ? 'selected' : '' ?>><?php echo array_values((array)$value)[1]; ?></option>
							<?php } ?>
						</select>
					...
					<div class="form-group">
						...
						<select multiple="multiple" name="how_it_happened_update[]" id="howItHappened" class="form-control	 multiple_reasons" required="required">
							...
							<option value="<?php echo array_values((array)$value)[0]; ?>" <?php echo in_array(array_values((array)$value)[0], $assoc_how) ? 'selected' : '' ?>><?php echo array_values((array)$value)[1]; ?></option>
							<?php } ?>
						</select>
					</div>
					<div class="form-group">
						<select multiple="multiple" name="sports_and_occupations_update[]" id="sportsOccupation" class="form-control multiple_sports_occ" required="required">
							...
							<option value="<?php echo array_values((array)$value)[0]; ?>"<?php echo in_array(array_values((array)$value)[0], $assoc_sports) ? 'selected' : '' ?>><?php echo array_values((array)$value)[1]; ?></option>
							<?php } ?>
						</select>
					</div>	
				</div>
				<div class="col-md-12">
					...
					<div class="form-group" id="hideField1">					
						<textarea name="progPlanUpdate" class="form-control" placeholder="Weekly Plan" ><?php echo $getProgramById[0]['weekly_plan'] ?></textarea>
					</div>
					<div class="form-group" id="hideField2">
						<textarea name="progLifestyleUpdate" class="form-control" placeholder="Lifestyle"><?php echo $getProgramById[0]['life_style'] ?></textarea>
					</div>
				</div>
			</div>
			<div class="row">
									...
									<div class="row">
										<div class="col-md-6">
											<div class="form-group">
												...
												<input type="text" name="phase[<?php echo $key ?>][nameUpdate]" placeholder="Phase Name" class="form-control nameEdit" value="<?php echo $value['name'] ?>">
											</div>
											<div class="form-group">
										...
										<div class="col-md-6">
											<div class="form-group">
												<input type="text" name="phase[<?php echo $key ?>][durationUpdate]" placeholder="Phase Duration" class="form-control duration" value="<?php echo $value['duration'] ?>">
											</div>
											<div class="form-group">
												...
												<ul class="sortable">
													<?php  
														foreach ($value['exercise'] as $keys => $values) { ?>																
															<li><span class="exerciseName">Exercise</span> <span class="deleteExerciseEdit glyphicon glyphicon-trash" data-tab-index="1" data-exercise-id="<?php echo $values['id'] ?>"></span>
																<div class="row">
																	<div class="col-md-6">
																		<div class="form-group">
																			<input type="hidden" class="id" name="phase[<?php echo $key ?>][exercise][<?php echo $keys ?>][exerciseId]" value="<?php echo $values['id'] ?>">
																			<input type="hidden" class="order" name="phase[<?php echo $key ?>][exercise][<?php echo $keys ?>][order]" value="<?php echo $values['order'] ?>">															
																			<input required="required" class="form-control set" type="text" placeholder="Sets x Reps"  name="phase[<?php echo $key ?>][exercise][<?php echo $keys ?>][sets]" value="<?php echo $values['sets_reps']?>">
																		</div>											
																		<div class="form-group">
																			<input required="required" class="form-control rest" type="text" placeholder="Rest"  name="phase[<?php echo $key ?>][exercise][<?php echo $keys ?>][rest]" value="<?php echo $values['rest'] ?>">
																		</div>
																		<div class="form-group">
																			<input required="required" class="form-control var" type="text"  placeholder="Variation" name="phase[<?php echo $key ?>][exercise][<?php echo $keys ?>][variation]" value="<?php echo $values['variation'] ?>">
																		</div>
																		<div class="form-group">
																			<textarea required="required" class="form-control equip"  placeholder="Equipment" name="phase[<?php echo $key ?>][exercise][<?php echo $keys ?>][equipment]"><?php echo $values['equipment'] ?></textarea>
																		</div>
																		<div class="form-group">
																			<textarea required="required" class="form-control ins"  placeholder="Special Instructions" name="phase[<?php echo $key ?>][exercise][<?php echo $keys ?>][instructions]"><?php echo $values['special_instructions'] ?></textarea>
																		</div>
																	</div>
																	<div class="col-md-6">
			...
			<div class="row">
				...
				<div class="col-md-6">
					...
					<div class="form-group">
						<div>
						   	<input type="button" name="upload-btn" id="upload-btn" class="button-secondary" value="Choose Image">
						    <input type="text" name="thumb" id="image_url" class="regular-text form-control" value="">
							<!-- <span id="imageWrapperPull" class="glyphicon glyphicon-triangle-bottom" data-toggle="collapse" data-target="#imageWrapper"></span>
							<div id="imageWrapper" class="collapse">
								<img src="">
							</div> -->
				...
				<div class="col-md-12">
					<div class="form-group">
						<textarea name="progDesc" class="form-control" placeholder="Description"></textarea>
					</div>
					<div class="form-group">
						<textarea name="progEquip" class="form-control" required="required" placeholder="Equipment"></textarea>
					</div>
					<div class="form-group" id="hideField1">					
						<textarea name="progPlan" class="form-control" placeholder="Weekly Plan"></textarea>
					</div>
					<div class="form-group" id="hideField2">
						<textarea name="progLifestyle" class="form-control" placeholder="Lifestyle"></textarea>
					</div>
				</div>
			</div>
			<div class="row">
							...
							<div id="phase1" class="tab-pane fade in active" data-tab-count="1">
								<div class="row">
									<div class="col-md-6">
										...
										<div class="form-group">
											<textarea required="required" type="text" name="phase[0][intro]" placeholder="Phase Introduction" class="form-control intro"></textarea>								
										</div>
									</div>
									<div class="col-md-6">
										<div class="form-group">
										...
										<div class="form-group">
											<textarea required="required" type="text" name="phase[0][notes]" placeholder="Phase Notes" class="form-control notes"></textarea>
										</div>
									</div>
									<div class="col-md-12">										
										<div class="exercises">
			...
			<div class="row">
				<div class="col-md-6">
					<select class="form-control" name="searchSelect">
						...
						<?php foreach ($body_parts as $key => $value){	?>
						<option value="<?php echo array_values((array)$value)[0] ?>"<?php echo (isset($_POST['search']) && $_POST['searchSelect'] == array_values((array)$value)[1]) ? 'selected' : '' ?>><?php echo array_values((array)$value)[1]; ?></option>
						<?php } ?>			
					</select>
				</div>
				<div class="col-md-6">
		...
		// if key is to edit program then this script will run, with 
			...
			<script type="text/javascript">
				...
				// adding value to input order
					...
					$('.multiple_parts').html(selectList);
					...
					selectList.sort(sort);
					$('.multiple_reasons').html(selectList)
					...
					selectList.sort(sort);
					$('.multiple_sports_occ').html(selectList)
					// showing phase area
...
<script type="text/javascript">
	jQuery(document).on('focusout', '[class*="name"]', function(){
